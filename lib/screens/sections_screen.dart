...
import 'package:quiz_app_supabase/screens/remote_notices_screen.dart';
...
class _SectionsScreenState extends State<SectionsScreen> {
  ...
  Future<void> _loadSections() async {
    try {
      ...
      final res = await _supabaseService.getSectionsByCategory(
        widget.category.id,
      );
  ...
  Widget build(BuildContext context) {
    final isSyllabusCategory = widget.category.name == 'Syllabus';
              ...
              children: [
                if (_showingCached)
                  Padding(
                    ...
                    child: Text(
                      'Offline mode: showing saved sections',
                      style: GoogleFonts.poppins(
                          ...
                          child: ListTile(
                            ...
                            onTap: () async {
                              ...
                              if (widget.category.name == 'Notice') {
                                final type = (section.name == 'Exam Notices')
                                    ? 'exam'
                                    : (section.name == 'Vacancy Notices')
                                    ? 'vacancy'
                                    : null;
                              ...
                              // Offline note: getQuestionCountBySection is still network-only.
                              // If offline, this may fail and you'll fall back to the catch.
                              try {
                                ...
                                // Fallback: if offline, just open quiz directly (non-batched).
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (_) =>
